...
Для конкретного запроса без конфигурации, указав третий параметр при выборе ресурса. Или вызвать метод `public()` после установки ресурса.

```javascript
api.res('user', 'default', false).load();
